...
use crate::{
    Editor, EditorSnapshot, PointForPosition, SemanticsProvider, ToggleInlayHints,
...
fn spawn_editor_hints_refresh(
    ...
    semantics_provider: std::rc::Rc<dyn SemanticsProvider>,
    ...
    cx.spawn(async move |editor, cx| {
        ...
        if editor
            .update(cx, |editor, cx| {
                let Some(inlay_hints) = &mut editor.inlay_hints else {
                    return;
                };
                ...
                for (row_range, new_hints_task) in dbg!(new_hint_tasks) {
                    ...
                    if known_chunks.insert(row_range.clone())
                        || ignore_previous_fetches
                        || invalidate_cache.should_invalidate()
                    {
                        hint_tasks.push(async move { (row_range, new_hints_task.await) });
                    }
                }
            })
            .is_err()
        ...
        editor
            .update(cx, |editor, cx| {
                ...
                if dbg!(invalidate_cache.should_invalidate()) {
                    hints_to_remove.extend(visible_inlay_hint_ids);
                }

                let hints_to_insert = dbg!(new_hints)
                    .into_iter()
                ...
                dbg!(("???????", &hints_to_remove, &hints_to_insert));
                            ...
                            move |params, _| {
                                ...
                                async move {
                                    ...
                                    Ok(Some(vec![lsp::InlayHint {
                                        ...
                                        label: lsp::InlayHintLabel::String(dbg!(
    ...
    async fn test_multiple_excerpts_large_multibuffer(cx: &mut gpui::TestAppContext) {
        ...
        editor
            .update(cx, |editor, _window, cx| {
                ...
                assert_eq!(
                    vec!["main hint(edited) #1".to_string()],
                    visible_hint_labels(editor, cx),
